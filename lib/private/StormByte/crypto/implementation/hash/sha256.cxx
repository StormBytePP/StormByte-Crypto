...
namespace {
	...
	ExpectedHashString ComputeSHA256(std::span<const std::byte> dataSpan) noexcept {
		try {
			...
			return Unexpected(HasherException("SHA-256 hashing failed: {}", e.what()));
		}
	}
}
...
ExpectedHashString SHA256::Hash(const FIFO& buffer) noexcept {
	DataType data;
...
Consumer SHA256::Hash(Consumer consumer) noexcept {
	Producer producer;
