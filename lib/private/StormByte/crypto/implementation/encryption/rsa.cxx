...
ExpectedCryptoFutureBuffer RSA::Encrypt(const StormByte::Buffers::Simple& input, const std::string& publicKey) noexcept {
	try {
		CryptoPP::AutoSeededRandomPool rng;
...
ExpectedCryptoFutureBuffer RSA::Decrypt(const StormByte::Buffers::Simple& encryptedBuffer, const std::string& privateKey) noexcept {
	try {
		CryptoPP::AutoSeededRandomPool rng;
...
ExpectedCryptoFutureString RSA::Decrypt(const std::string& message, const std::string& publicKey) noexcept {
	return StormByte::Unexpected<Exception>("Decrypting with a public key is not supported.");
}
