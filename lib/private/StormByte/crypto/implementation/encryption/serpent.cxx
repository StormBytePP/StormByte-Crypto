...
ExpectedCryptoBuffer Serpent::Encrypt(const FIFO& input, const std::string& password) noexcept {
	DataType data;
...
Consumer Serpent::Encrypt(Consumer consumer, const std::string& password) noexcept {
	Producer producer;
...
ExpectedCryptoBuffer Serpent::Decrypt(const FIFO& input, const std::string& password) noexcept {
	DataType data;
	auto read_ok = input.Read(data);
	if (!read_ok.has_value()) {
	return Unexpected(CrypterException("Failed to extract data from input buffer"));
...
Consumer Serpent::Decrypt(Consumer consumer, const std::string& password) noexcept {
	Producer producer;
